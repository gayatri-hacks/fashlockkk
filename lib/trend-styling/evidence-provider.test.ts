...
test("Serper HTTP 400 reports a sanitized invalid-request code and bounded reason", async () => {
...
test("Serper retry metadata defers the same claimed research job without another provider call", async () => {
  ...
  const store: ResearchStore = {
    ...
    async insertEvidence() { throw new Error("unexpected evidence insert"); },
    async complete() { throw new Error("unexpected completion"); },
    async retry(_job, _message, nextRetry) { retryAfter = nextRetry; },
  };
  const result = await runResearchWorker({
    ...
    searchProvider: {
      async search() {
        searches += 1;
        throw new SerperEvidenceSearchError("quota_or_rate_limit", 429, "rate_limited", "Too many searches", 120);
      },
    },
    now,
  });
  ...
  assert.equal(retryAfter, "2026-07-20T00:02:00.000Z");
});
