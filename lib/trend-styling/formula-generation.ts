...
export function buildEvidenceGroundedFormulaPrompt(input: {
  ...
  // Formula validation requires the independent supporting sources to be fresh
  // within 60 days. Only expose evidence that can satisfy that same contract so
  // the provider cannot build an otherwise well-formed batch from stale rows.
  const cutoff = new Date((input.now || new Date()).getTime() - 60 * 86_400_000);
  ...
  return `Create six evidence-grounded outfit formulas. Do not infer from the keyword alone. Every formula must cite at least two supplied evidence_ids for its own audience, and those IDs must come from at least two different source_domain values. Never invent, alter or duplicate an evidence ID. Within each audience, easy_entry, current_uniform and editorial_push must use materially different garment combinations; no two slots may share 75% or more of their garment-name tokens. Name every garment precisely: never use vague garment names such as "piece", "layer" or "top". Make the canonical trend visibly present in every formula and specify a complete outfit with footwear and accessories. Each rationale must explain the specific silhouette, proportion, material or colour relationship supported by its cited evidence; never use generic claims such as "versatile", "elevates the look", "perfect balance" or "adds interest". Every evidence-based rationale must cite supplied evidence_ids. Treat the concept, keyword, market list and evidence hash as application context only: do not copy them into response metadata and do not generate any owner, authority, review, ID, hash, type or publication fields.\nCanonical keyword: ${input.canonicalKeyword}\nSeason guidance: ${input.season}\nMarket context: ${input.requestingMarket}\nStrongest markets: ${input.marketPlan.strongestMarkets.join(", ")}\nResearch markets: ${input.marketPlan.researchMarkets.join(", ")}\nEvidence: ${JSON.stringify(compactEvidence)}`;
}
